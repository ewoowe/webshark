...
func buildCaptureCommand(host *gorm.Host, capture HostSingleCapture, pcapPath string, onlyCapture bool) (*exec.Cmd, string) {
	...
	// 使用 sudo 提升权限以允许抓包
	// tcpdump 命令 + 父进程监控：当 SSH 连接断开时自动终止 tcpdump
	tcpdumpCmd := fmt.Sprintf(`sudo -n tcpdump %s -U -w - %s & TPID=$!; SHELL_PID=$$; ORIG_PPID=$(ps -o ppid= -p $SHELL_PID | tr -d " "); sleep 0.5; echo "[$(date)] INIT: SHELL_PID=$SHELL_PID ORIG_PPID=$ORIG_PPID TPID=$TPID" >> /tmp/tcpdump_debug.log; while true; do NEW_PPID=$(ps -o ppid= -p $SHELL_PID 2>/dev/null | tr -d " "); if [ "$NEW_PPID" != "$ORIG_PPID" ]; then echo "[$(date)] DETECT: PPID changed from $ORIG_PPID to $NEW_PPID, killing TPID=$TPID and children" >> /tmp/tcpdump_debug.log; PIDS_TO_KILL=$(pgrep -P "$TPID" 2>/dev/null; echo "$TPID"); echo "[$(date)] DETECT: PIDS_TO_KILL=$PIDS_TO_KILL" >> /tmp/tcpdump_debug.log; echo "$PIDS_TO_KILL" | while read pid; do [ -n "$pid" ] && { sudo -n kill -KILL $pid 2>/dev/null && echo "[$(date)] KILL: SIGKILL sent to pid $pid" >> /tmp/tcpdump_debug.log || echo "[$(date)] KILL: failed to kill pid $pid" >> /tmp/tcpdump_debug.log; }; done; echo "[$(date)] EXIT: script exiting" >> /tmp/tcpdump_debug.log; exit 0; fi; sleep 1; done; echo "[$(date)] EXIT: script exiting normally" >> /tmp/tcpdump_debug.log; wait`, interfaceArg, bpfFilter)
