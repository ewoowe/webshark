...
type Task struct {
	...
	Status          string      `gorm:"column:status;type:varchar(64);default:running;comment:任务状态，created，running，failed, stopping, stopped" json:"status"`
	Message         string      `gorm:"column:message;type:varchar(1024);comment:任务状态下的额外消息，以;分隔" json:"message"`
}
